...
| C8 | "End-to-end checkout settles USDC on Arbitrum and anchors a proof receipt on Base." | invoice `70bfbad2-aafa-4a45-a793-d21b1ce64f1c` ; settle tx `0xce43e057...3550b` ; proof tx `0xff365c09...6715e` | 2026-06-20 | med | depends on Particle UA same-chain transfer + Supabase verifier |
...
- ⚠️ "OneLink Pay supports cross-chain value movement via the Universal Account." — now PROVEN at
  the primitive level (C21: a live cross-chain payment to the merchant on v2-beta.3, independently
  verified on-chain). The old `REFUND_FAILED` / `-32613` / `-32801` failures are resolved. SCOPE
  HONESTLY: proven in the cross-chain proof lab, NOT yet in the product `/pay` checkout — do not
  claim the live product does cross-chain settlement until `/pay` is wired to
  `createUniversalTransaction` and re-proven.
