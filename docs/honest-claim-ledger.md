...
| C8 | "End-to-end checkout settles USDC on Arbitrum and anchors a proof receipt on Base." | invoice `70bfbad2-aafa-4a45-a793-d21b1ce64f1c` ; settle tx `0xce43e057...3550b` ; proof tx `0xff365c09...6715e` | 2026-06-20 | med | depends on Particle UA same-chain transfer + Supabase verifier. ⚠️ 2026-06-21: the 7702 `/pay` path was switched from `createTransferTransaction` to `createUniversalTransaction` (C21/Step 3) — this same-chain claim was proven on the OLD path and is NOT re-verified on the new one; re-run a same-chain `/pay` before reusing C8. |
...
- ⚠️ "OneLink Pay supports cross-chain value movement via the Universal Account." — PROVEN end-to-end
  ...
  are resolved. SCOPE HONESTLY: proven on the local feature branch with REAL mainnet transactions,
  NOT yet deployed to prod (prod still runs the prior single-chain build) — say "proven in our
  checkout (real mainnet)", not "live in production", until the v2 `/pay` is deployed. Frame the
  cross-chain settlement as Particle Universal Accounts' chain abstraction, which it is.
